...
def parse_input(data: str):
    ...
    pic = np.array([hashperiod_to_binarray(line) for line in pic_raw.splitlines()], dtype=np.int8)
    return key, pic

def shifts(mx: np.ndarray) -> np.ndarray:
    x = mx.shape[0] - 2
    y = mx.shape[1] - 2
    return np.stack([mx[i:i+x, j:j+y] for i in range(3) for j in range(3)], axis = -1)
...
def to_string(mx: np.ndarray) -> str:
    return '\n'.join([''.join(map(str, map(lambda x: "#" if x == 1 else "." if x == 0 else "?", line))) for line in mx])
