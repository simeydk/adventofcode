...
def neighbour_coords(coords: Coords, mx: np.ndarray) -> List[Coords]:
    ...
    results: List[Coords] = [Coords(x+i, y+j) for i in [-1, 0, 1] for j in [-1, 0, 1] if not (i == 0 and j == 0)]
    # exclude out of bounds coords
    results = [c for c in results if (0 <= c.x < mx.shape[0] and 0 <= c.y < mx.shape[1])]
    return results
